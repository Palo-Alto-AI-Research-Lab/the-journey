...
**Почему (грабли, пойманные 2026-06-25):** `.stignore` в Syncthing **пер-машинный и НЕ синкается**. Достаточно, чтобы ОДНА машина не добавила ignore родителя ИЛИ не приняла вложенный шар — и для неё шина обрывается в ОБЕ стороны, пока все прочие папки показывают 100%. Хаб имел отдельный шар + ignore; один из ноутов вложенный шар **не принял** И его волт `_machine-bus` не игнорировал → ноут замёрз, хотя волт/скиллы/home были 100% (выглядело как «робот шины мёртв» — неверно, труба была перерезана). Машины остальных напарников шар **приняли** (100%) → у них была только поломка чтения, не синка.
