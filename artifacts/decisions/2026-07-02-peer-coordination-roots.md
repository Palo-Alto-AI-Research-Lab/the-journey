...
- #0b1d5d4f — план по рецидиву F5VYGLV ноута + osc-риск сторожей (tier 2).
- Дубль-группа с Русланой (инцидент 25.06) — удалить/оставить, висит с 25.06.

## Физически на людях
- Мак Русланы: робот мёртв 7 дней — нужно открыть Мак/перезапустить launchd (Руслана).
- Наталья: `claude /login` + доставить cron_watchdog модуль (робот жив, доставку сделает хаб через _transit, логин — она).
